...
#servers url
preview_path = 'background/previews/'
download_path = 'background/download/'
tab_path = 'background/tab/previews/'
...
def get_server_category_excel(source_path, dest_path):
    infos = ft.excel_to_json(source_path, 0)
    dataFrame = ps.DataFrame(columns={'id',	'name',	'vipState','online','sort','opId'})
    infos_new = []
    for info in infos:
        info_new = {}
        for key in dataFrame.columns:
